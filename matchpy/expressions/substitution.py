...
class Substitution(Dict[unicode, VariableReplacement]):
    ...
    def extract_substitution(self, subject, pattern):
        ...
        if getattr(pattern, u'variable_name', False):
            ...
            for subj, patt in izip(op_expression, pattern):
    ...
    def _match_value_repr_str(value
        ...
        if isinstance(value, (list, tuple)):
            return u'({!s})'.format(u', '.join(unicode(x) for x in value))
        if isinstance(value, Multiset):
            return u'{{{!s}}}'.format(u', '.join(unicode(x) for x in sorted(value)))
