...
T = TypeVar(u'T')
VariableWithCount = NamedTuple(u'VariableWithCount', [(u'name', unicode), (u'count', int), (u'minimum', int)])
...
def weak_composition_iter(n, num_parts):
    ...
    for t in itertools.combinations(xrange(m), num_parts - 1):
        yield tuple(v - u - 1 for u, v in izip(first + t, t + last))
...
def _make_variable_generator_factory(value, total, variables):
    ...
    def _factory(subst):
        ...
        for solution in solutions:
            ...
            for var, count in izip(variables, solution):
...
def get_short_lambda_source(lambda_func):
    ...
    lambda_node = (node for node in ast.walk(source_ast) if isinstance(node, ast.Lambda)), None.next()
...
def extended_euclid(a, b):
    u"""Extended Euclidean algorithm that computes the Bézout coefficients as well as :math:`gcd(a, b)`
...
def base_solution_linear(a, b, c):
    u"""Yield solutions for a basic linear Diophantine equation of the form :math:`ax + by = c`.
    ...
    while True:
        try:
            while generator_index < generator_count:
                ...
                next_data = generators[generator_index].next()
                generator_index += 1
            yield next_data
            generator_index -= 1
