# -*- coding: utf-8 -*-
u"""This module contains the CommutativePatternsParts class which is used by multiple matching algorithms."""
...
__all__ = [u'CommutativePatternsParts', u'Matcher', u'VarInfo']

Matcher = Callable[[Sequence[Expression], Expression, Substitution], Iterator[Substitution]]
VarInfo = NamedTuple(u'VarInfo', [(u'min_count', int), (u'type', Optional[type])])


class CommutativePatternsParts(object):
    u"""Representation of the parts of a commutative pattern expression.
    ...
    def __init__(self, operation, *expressions):
        u"""Create a CommutativePatternsParts instance.
            ...
            if is_constant(expression):
                ...
                if wc.variable_name:
                    ...
                    if wc.fixed_size:
                        ...
                        symbol_type = getattr(wc, u'symbol_type', None)
    ...
    def __str__(self):
        parts = []
        parts.extend(imap(unicode, self.constant))
        parts.extend(imap(unicode, self.syntactic))
        parts.extend(imap(unicode, self.rest))
        ...
        return u'{}({})'.format(self.operation.name, u', '.join(parts))
