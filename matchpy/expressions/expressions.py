# -*- coding: utf-8 -*-
u"""This module contains the expression classes.
...
ExprPredicate = Optional[Callable[[u'Expression'], bool]]
ExpressionsWithPos = Iterator[Tuple[u'Expression', Tuple[int, ...]]]
...
class Expression(object):
    u"""Base class for all expressions.
    ...
    @cached_property
    def variables(self):
        u"""A multiset of the variables occurring in the expression."""
        variables = Multiset()
        self.collect_variables(variables)
        return variables

    def collect_variables(self, variables):
        u"""Recursively adds all variables occuring in the expression to the given multiset.
    ...
    @cached_property
    def symbols(self):
        u"""A multiset of the symbol names occurring in the expression."""
        symbols = Multiset()
        self.collect_symbols(symbols)
        return symbols

    def collect_symbols(self, symbols):
        u"""Recursively adds all symbols occuring in the expression to the given multiset.
    ...
    def is_constant(self):
        u"""True, iff the expression does not contain any wildcards."""
    ...
    def is_syntactic(self):
        u"""True, iff the expression does not contain any associative or commutative operations or sequence wildcards."""
    ...
    def with_renamed_vars(self, renaming):
        u"""Return a copy of the expression with renamed variables."""
        raise NotImplementedError()

    def preorder_iter(self, predicate=None):
        u"""Iterates over all subexpressions that match the (optional) `predicate`.
    ...
    def __getitem__(self, position):
        u"""Return the subexpression at the given position(s).
...
class Operation(Expression):
    ...
    def __str__(self):
        if self.infix:
            ...
            value = u'({!s})'.format(separator.join(unicode(o) for o in self.operands))
        else:
            value = u'{!s}({!s})'.format(self.name, u', '.join(unicode(o) for o in self.operands))
        if self.variable_name:
            value = u'{}: {}'.format(self.variable_name, value)
        return value

    def __repr__(self):
        operand_str = u', '.join(imap(repr, self.operands))
        if self.variable_name:
    ...
    def __lt__(self, other):
        ...
        for left, right in izip(self.operands, other.operands):
            if left < right:
                return True
            elif right < left:
                return False
    ...
    def __eq__(self, other):
        if not isinstance(other, type(self)):
            return NotImplemented
        return (
            len(self.operands) == len(other.operands) and all(x == y for x, y in izip(self.operands, other.operands)) and
...
class CommutativeOperation(object):
    __metaclass__ = ABCMeta
    @classmethod
    def __subclasshook__(cls, C):
...
class Pattern(object):
    ...
    def __str__(self):
        ...
        return u'{} /; {}'.format(self.expression, u' and '.join(imap(unicode, self.constraints)))
