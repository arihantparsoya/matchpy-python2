...
__all__ = [u'match', u'match_anywhere']


def match(subject, pattern):
    ur"""Tries to match the given *pattern* to the given *subject*.
    ...
    if not is_constant(subject):
        raise ValueError(u"The subject for matching must be constant.")
...
def match_anywhere(subject, pattern):
    u"""Tries to match the given *pattern* to the any subexpression of the given *subject*.
...
def _match(subjects, pattern, subst,
           constraints):
    match_iter = None
        ...
        if getattr(pattern, u'variable_name', False):
            for new_subst in match_iter:
                try:
                    ...
                    for i in _check_constraints(new_subst, constraints):
                        yield i
        else:
            for i in match_iter:
                yield i
...
def _match_factory(expressions, operand, constraints, matcher):
    def factory(subst):
        for i in matcher(expressions, operand, subst, constraints):
            yield i
...
def _build_full_partition(sequence_var_partition, subjects,
                          operation):
    u"""Distribute subject operands among pattern operands.
...
def _non_commutative_match(subjects, operation, subst, constraints, matcher):
    ...
    for part in weak_composition_iter(remaining, sequence_var_count):
        ...
        factories = [_match_factory(e, o, constraints, matcher) for e, o in izip(partition, operation)]
...
def _match_operation(expressions, operation, subst, matcher, constraints):
    ...
    if not isinstance(operation, CommutativeOperation):
        for i in _non_commutative_match(expressions, operation, subst, constraints, matcher):
            yield i
    else:
        parts = CommutativePatternsParts(type(operation), *operation)
        for i in _match_commutative_operation(expressions, parts, subst, constraints, matcher):
            yield i
            ...
            if issubclass(pattern.operation, AssociativeOperation):
                for v in fixed_vars.distinct_elements():
                    ...
                    if len(value) > l:
                        ...
                        normal.add(wrapped)
                        sequence_subst[v] = normal if l > 1 else iter(normal).next()
                    else:
                        assert len(value) == 1 and l == 1, u"Fixed variables with length != 1 are not supported."
    ...
    def factory(data):
        ...
        if variable_name in substitution:
            ...
            if length == 1:
                ...
                assert variable_name is None, u"Fixed variables with length != 1 are not supported."
