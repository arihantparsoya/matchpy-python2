# -*- coding: utf-8 -*-
u"""This module contains various many-to-one matchers for syntactic patterns:
...
import itertools
#from repr import recursive_repr
...
__all__ = [u'FlatTerm', u'is_operation', u'is_symbol_wildcard', u'DiscriminationNet', u'SequenceMatcher']

T = TypeVar(u'T')

OPERATION_END = u')'
u"""Constant used to represent the end of an operation in a :class:`FlatTerm` and :class:`DiscriminationNet`."""

EPSILON = u'ε'
u"""Constant used to label an epsilon transition for the :class:`DiscriminationNet`."""


def is_operation(term):
    u"""Return True iff the given term is a subclass of :class:`.Operation`."""
    return isinstance(term, type) and issubclass(term, Operation)


def is_symbol_wildcard(term):
    u"""Return True iff the given term is a subclass of :class:`.Symbol`."""
    return isinstance(term, type) and issubclass(term, Symbol)


def _get_symbol_wildcard_label(state, symbol):
    u"""Return the transition target for the given symbol type from the the given state or None if it does not exist."""
    return (t for t in state.keys() if is_symbol_wildcard(t) and isinstance(symbol, t)), None.next()
...
class FlatTerm(Sequence[TermAtom]):
    u"""A flattened representation of an :class:`.Expression`.
    ...
    __slots__ = u'_terms', u'_is_syntactic'
    ...
    @slot_cached_property(u'_is_syntactic')
    def is_syntactic(self):
        u"""True, iff the flatterm is :term:`syntactic`."""
    ...
    def merged(cls, *flatterms):
        u"""Concatenate the given flatterms to a single flatterm.
    ...
    @classmethod
    def _flatterm_iter(cls, expression):
        u"""Generator that yields the atoms of the expressions in prefix notation with operation end markers."""
        if isinstance(expression, Operation):
            yield type(expression)
            for operand in expression:
                for i in cls._flatterm_iter(operand):
                    yield i
            yield OPERATION_END
            ...
            assert False, u"Unreachable unless a new unsupported expression type is added."

    @staticmethod
    def _combined_wildcards_iter(flatterm):
        u"""Combine consecutive wildcards in a flatterm into a single one."""
    ...
    def __str__(self):
        return u' '.join(imap(_term_str, self))

    def __repr__(self):
        return u'[{!s}]'.format(u', '.join(imap(unicode, self)))


def _term_str(term):  # pragma: no cover
    u"""Return a string representation of a term atom."""
    if is_operation(term):
        return term.name + u'('
    elif is_symbol_wildcard(term):
        return u'*{!s}'.format(term.__name__)
    elif isinstance(term, Wildcard):
        return u'*{!s}{!s}'.format(term.min_count, (not term.fixed_size) and u'+' or u'')
    elif term == Wildcard:
        return u'*'
    else:
        return unicode(term)
...
class _State(Dict[TransitionLabel, u'_State'], Generic[T]):
    u"""An DFA state used by the :class:`DiscriminationNet`.
    ...
    def __init__(self, payload=None):
        super(_State, self).__init__(self)
        self.id = _State._id
        _State._id += 1
        self.payload = payload if payload is not None else []

    def _target_str(self, value):  # pragma: no cover
        u"""Return a string representation of a transition target."""
        if value is self:
            return u'self'
        else:
    ...
    #@recursive_repr()
    def __repr__(self):
        return u'{{STATE {!s}: {!s}}}'.format(
            self.payload,
            u', '.join(u'{!s}:{!s}'.format(_term_str(term), self._target_str(target)) for term, target in self.items())
        )


class _StateQueueItem(Generic[T]):
    u"""Internal data structure used by the product net algorithm.
    ...
    @property
    def labels(self):
        u"""Return the set of transition labels to examine for this queue state.
    ...
    def __repr__(self):
        return u'NQI({!r}, {!r}, {!r}, {!r}, {!r}, {!r})'.format(
            self.id1, self.id2, self.depth, self.fixed, self.state1, self.state2
        )


class DiscriminationNet(Generic[T]):
    u"""An automaton to distinguish which patterns match a given expression.
    ...
    def add(self, pattern, final_label=None):
        u"""Add a pattern to the discrimination net.
    ...
    @staticmethod
    def _create_child_state(state, label):
        new_state = _State()
        state[label] = new_state
        return new_state

    @classmethod
    def _generate_syntactic_net(cls, flatterm, final_label):
        root = state = _State()
    ...
    @classmethod
    def _generate_state_chain(cls, state, label, count):
        for _ in xrange(count):
            state = cls._create_child_state(state, label)
        return state

    @classmethod
    def _generate_net(cls, flatterm, final_label):
        u"""Generates a DFA matching the given pattern."""
        ...
        for term in flatterm:
            ...
            if isinstance(term, Wildcard):
                ...
                for _ in xrange(term.min_count):
                ...
                if is_operation(term):
                    ...
                    if last_wildcards[-1] or fail_states[-1]:
                        ...
                        if term.arity.fixed_size:
                            ...
                            for _ in xrange(term.arity.min_count):
    ...
    @staticmethod
    def _create_state(state_set, states):
        payload = set()
    ...
    @classmethod
    def _convert_nfa_to_dfa(cls, root, states):
        new_root = cls._epsilon_closure(set([root.id]), states)
        queue = [new_root]
    ...
    @staticmethod
    def _epsilon_closure(state, states):
        output = set(state)
    ...
    @classmethod
    def _target_set(cls, state, label, states):
        output = set()
    ...
    @staticmethod
    def _get_next_state(state, label, fixed):
        if fixed:
            return state, False
    ...
    def _match(self, subject, collect=False):
                ...
                try:
                    try:
                        ...
                        if is_operation(term):
                            ...
                            raise TypeError(u"Subject {} contains non-terminal atom: {}".format(subject, term))
                except KeyError:
    ...
    def match(self, subject):
        u"""Match the given subject against all patterns in the net.
    ...
    def is_match(self, subject):
        u"""Check if the given subject matches any pattern in the net.
        ...
        """
        try:
            self.match(subject).next()
        except StopIteration:
            return False
        return True

    def as_graph(self):  # pragma: no cover
        u"""Renders the discrimination net as graphviz digraph."""
        if Digraph is None:
        ...
        while queue:
            ...
            if not state.payload:
                dot.node(u'n{!s}'.format(state.id), u'', {u'shape': (u'circle' if state else u'doublecircle')})
            else:
                dot.node(u'n{!s}'.format(state.id), u'\n'.join(imap(unicode, state.payload)), {u'shape': u'box'})
        ...
        while queue:
            ...
            for (label, other) in state.items():
                dot.edge(u'n{!s}'.format(state.id), u'n{!s}'.format(other.id), _term_str(label))
...
class SequenceMatcher(object):
    ur"""A matcher that matches many :term:`syntactic` patterns in a surrounding sequence.
    ...
    __slots__ = (u'_net', u'_patterns', u'operation')
    ...
    def add(self, pattern):
        u"""Add a pattern that will be recognized by the matcher.
        ...
        if self.operation is None:
            if not isinstance(inner, Operation) or isinstance(inner, CommutativeOperation):
                raise TypeError(u"Pattern must be a non-commutative operation.")
            self.operation = type(inner)
        elif not isinstance(inner, self.operation):
            raise TypeError(
                u"All patterns must be the same operation, expected {} but got {}".format(self.operation, type(inner))
            )

        if len(inner) < 3:
            raise ValueError(u"Pattern has not enough operands.")
    ...
    def _check_wildcard_and_get_name(operand):
        if not isinstance(operand, Wildcard) or operand.fixed_size or operand.min_count > 0:
            raise ValueError(u'Expected a star wildcard, got {!s}.'.format(operand))

        return operand.variable_name

    @classmethod
    def can_match(cls, pattern):
        u"""Check if a pattern can be matched with a sequence matcher.
    ...
    def match(self, subject):
        u"""Match the given subject against all patterns in the sequence matcher.
        ...
        for i in xrange(len(flatterms)):
            ...
            for index in self._net._match(flatterm, collect=True):
                ...
                if not all(itertools.starmap(substitution.extract_substitution, izip(expr_operands, patt_operands))):
                    continue
    ...
    def as_graph(self):  # pragma: no cover
        u"""Renders the underlying discrimination net as graphviz digraph."""
